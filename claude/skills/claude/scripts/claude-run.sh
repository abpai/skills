...
review adds Edit, Write, and NotebookEdit to disallowed tools.
...
for number in "$HEARTBEAT_SECONDS" "$STALL_TIMEOUT_SECONDS" "$REPORT_TIMEOUT_SECONDS" "$TIMEOUT_SECONDS" "$TERM_GRACE_SECONDS"; do
...
load_continue_defaults() {
  ...
  done < <(read_env_values "$prior" WORKSPACE RUN_ROOT SESSION_ID PERMISSION_MODE READ_ONLY MODEL EFFORT HEARTBEAT_SECONDS STALL_TIMEOUT_SECONDS REPORT_TIMEOUT_SECONDS TIMEOUT_SECONDS NO_SESSION_PERSISTENCE)
...
claude_cmd=(claude -p --output-format stream-json --verbose --include-partial-messages --include-hook-events --permission-mode "$PERMISSION_MODE")
...
extract_final() {
  python3 - "$EVENTS_LOG" "$FINAL_MESSAGE" <<'PY'
import json, os, sys
from pathlib import Path
...
printf '[claude-run] event=start run_id=%s mode=%s session_id=%s workspace=%q\n' "$RUN_ID" "$MODE" "$SESSION_ID" "$WORKSPACE"
...
if ! command -v claude >/dev/null 2>&1; then
  write_status failed 127 0 failed
  RUN_FINALIZED="true"
  rm -f "$CONTINUE_SCRIPT"
  echo "[claude-run] event=finish state=failed exit_code=127 detail=claude-not-found"
  exit 127
fi
