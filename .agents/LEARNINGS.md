...
| 2026-04-28 | self   | Used `path` as a zsh loop variable while downloading upstream skill files, which clobbered zsh's command search path | In zsh snippets, avoid special tied variable names like `path`; use names such as `rel` or `file_path` instead |
| 2026-04-28 | self   | Preserved upstream `disable-model-invocation` frontmatter on an imported skill, which `skills-ref validate` rejects | Strip non-spec upstream frontmatter before finishing imports, then run `skills-ref validate <skill-dir>` on each new skill |
...
| 2026-04-12 | self   | Pi execute can strand a task in `in_progress` when the coordinator waits on a foreground generator/reviewer before persisting the attempt/handoff state, and dynamic preflight then keeps resuming that stale snapshot | Before waiting on a foreground Pi subagent, persist the next `build_pass` plus a durable `current_step`/pending-stage marker (or handoff artifact path) so resume can continue from review/evaluation instead of re-entering a half-finished build |
| 2026-04-12 | self   | Pi state-root docs drifted from real multi-run needs, which left command preflights, README examples, and local `.agents/pi/` layout disagreeing about where active state lives | When changing Pi state layout, update protocol docs, command preflights, README diagrams, ignore rules, version metadata, and the repo's actual `.agents/pi/` tree in the same pass |
| 2026-04-13 | self   | The first multi-run Pi pass treated `contracts/` as transient and mixed `state_root/` path literals with prose-style `state_root` references, which made the commit story and coordinator wording inconsistent | Keep `contracts/` with the durable plan artifacts unless there's a strong reason not to, and pick one `state_root` phrasing convention across command docs and protocol prose |
| 2026-04-13 | self   | Added Gemini support to Pi docs and agents but left `allowed-tools`/published metadata on the old Codex-only assumptions | When adding a new external CLI to a skill or plugin, update every runnable frontmatter allowlist plus `versions.json` and marketplace-facing descriptions in the same change |
