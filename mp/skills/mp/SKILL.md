...
This plugin exposes one public Codex skill surface for the Matt Pocock-inspired workflow family. Claude Code also exposes command wrappers under `/mp:*`. The individual workflow modules live as flat Markdown files under `internal/` so they are bundled with the plugin without becoming separate installable skills or per-workflow folders.
