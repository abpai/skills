...
Define or audit a coding goal so it is specific, bounded, and verifiable before an agent starts work.

Use this module when the user asks to turn a rough coding ask into a goal, verify whether a ticket or plan is ready for autonomous execution, define done, or make acceptance criteria concrete. This is different from `improve-prompt`: the goal is not prettier wording, but an executable contract with a verifier, scope, stop conditions, and output shape.

The animating principle: if the goal cannot tell the agent how to know it is done, where to stop, and what evidence to leave, the human becomes the verifier. A good goal preempts that.

## How to run this module

### 1. Read what you have

`$ARGUMENTS` (or recent conversation) contains the rough goal, ticket, task, or plan. Classify:
...
**Cap at round 5.** If still under threshold, hand back a partial brief that flags the unscored dimensions inline as `[NEEDS: ...]` so the user can fill them before execution. Mark it `NOT READY`.

**Refuse autonomy when:** V cannot reach ≥1 after probing — the goal is not autonomy-tractable. Recommend `improve-prompt` for a sharper one-shot, or downgrade to candidates mode (see references/verification.md §5).
...
### 3. Rewrite into the goal brief skeleton

Use the skeleton below. Then return:

1. **Readiness verdict** — `READY`, `NOT READY`, or `HUMAN_DECISION`.
2. **The goal brief** — ready to paste when `READY`; marked with `[NEEDS: ...]` when not ready.
3. **Goal checklist** — verifier defined / scope bounded / context named / stop conditions set.
4. **Final clarity scorecard** — V/S/C/X/O scores so the user sees which dimensions are concrete and which still need work.

Lead with the brief. Keep commentary minimal.

---

## The goal brief skeleton
...
> Verification commands and their expected outcomes are fixed at the start of the goal. You may not edit, skip, weaken, delete, `.skip`, `xfail`, `// @ts-ignore`, `# type: ignore`, `eslint-disable`, or `|| true` a check to make it pass. If a check is wrong, stop and report `BLOCKED` — do not silently rewrite it.
...
- [references/verification.md](references/verification.md) — verification taxonomy by goal shape, exit-criteria clauses, SITREP format, and what to do when verification is genuinely hard. Use this while filling in the **Done when** and **Acceptance checks** sections.
...
```md
# Goal: Add rate limiting to GET /api/menu and prove it works
...
## SITREP
GOAL_REPORT.md with STATUS, CHANGED, VERIFIED (paste output of all 3 checks), ASSUMPTIONS, UNRESOLVED, NEXT.
```
