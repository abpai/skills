...
4. For each primitive, spawn **both** `claude-researcher` and
...
5. When all researchers complete, read all result files from `research/fanout/`.
6. Build a **consensus matrix**: compare Claude vs Codex recommendations per
...
7. Present the matrix to the user and collect tiebreak decisions.
8. Write `research/consensus-matrix.md`.

### Phase C — Task Proposal (foreground planner)

9. Spawn a **fresh** `planner` (foreground) with the primitives and resolved
   tech decisions as context. The planner proposes ordered task slices with
   specific test criteria.
10. The planner presents tasks to the user for confirmation.

### Phase D — Iterative Codex Review (coordinator-driven)

11. Spawn `codex-reviewer` pass 1: review brief + tasks for gaps, risks, and
    test adequacy. Save to `reviews/codex-plan-pass-1.json`.
    - Incorporate `must_address` items into the plan.
    - Note `nice_to_have` items.
12. If pass 1 found issues: spawn `codex-reviewer` pass 2 on the updated plan.
    Save to `reviews/codex-plan-pass-2.json`.
    - If clean (`changed: false`), skip pass 3.
13. If pass 2 found issues: spawn `codex-reviewer` pass 3. Remaining issues
...
14. Present the final plan to the user: brief summary, consensus matrix,
    codex review results, ordered tasks, noted risks.
15. On approval, write: `brief.md`, `rubric.json`, `tasks/*.json`, and
