...
Patterns that secretly require a human in the loop. Scan the user's draft for these, apply the rewrite. The goal is one-shot autonomy — the user fires the brief and walks away.
...
**Rewrite:** Paste the invariant verbatim into the brief: "Every X must Y. Verify by [grep/test]." If you can't articulate it, the task isn't ready to fire.
...
**Trap:** Polite phrasing smuggles a second task in; doubles scope, doubles failure surface.
**Detect:** "also", "while you're at it", "might as well", "by the way".
**Rewrite:** Pull the secondary task into a separate brief, or add: "Out of scope: [list]. Note any incidental cleanup in SITREP under DEFERRED; do not commit it."
