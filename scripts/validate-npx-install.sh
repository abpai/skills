...
for skill_file in "${SKILL_FILES[@]}"; do
  ...
  out="$(INSTALL_INTERNAL_SKILLS=1 "${SKILLS_CMD[@]}" add "$skill_dir" --list -y 2>&1 | strip_ansi || true)"

  if grep -qiE 'No skills found' <<<"$out" || ! grep -qiE 'Found [1-9][0-9]* skill' <<<"$out"; then
    echo "  [FAIL] $skill_file: installer did not discover this skill"
