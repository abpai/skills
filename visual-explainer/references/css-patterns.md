...
Shared CSS that all templates use. Templates remain self-contained for copy-paste, but this is the canonical source for the common foundation.
...
### Base Card Depth Tiers
See the `.ve-card`, `.ve-card--elevated`, `.ve-card--recessed`, and `.ve-card--hero` classes in the Section / Card Components section below.

Templates should start from the Theme Setup variables below plus these shared patterns.
...
### CSS — Mobile (sticky horizontal bar)
