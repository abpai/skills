---
name: harness
description: "Route agent-harness workflows through one scoped /harness command. Use baseline to inventory and characterize existing production behavior at repo scale, docs to make a repo ergonomic for agent-driven development, doctor for verification-first readiness and diff audits, compliant to overhaul a repo end-to-end, capture to pin one behavior surface, onboard to emit an autonomous-ready manifest, evals to seed eval cases, and dogfood to harden a skill by using it under review."
argument-hint: "[subcommand] [args] - e.g. baseline, docs, doctor, compliant, capture, onboard, evals, dogfood"
metadata:
  version: "1.6.0"
---
...
Known subcommands: `baseline`, `docs`, `doctor`, `compliant`, `capture`, `onboard`, `evals`, `dogfood`.
...
- Use `capture.md` to characterize one current behavior surface with tests/snapshots **before** an agent changes legacy or under-tested code — the safety net that lets an agent tell a fix from a regression. It also has row mode for `baseline.md` (`BehaviorRow` in, `LedgerRow` out). Outputs a behavior ledger entry and a coverage-gap report.
...
These workflows form the agent-driven pipeline end to end: `baseline` inventories and pins existing production behavior → `capture` pins one scoped behavior when needed → `docs`/`compliant` make the repo ergonomic → `doctor` audits readiness and diff self-review → `onboard` emits the manifest and `evals` seeds the tests a factory runs → `dogfood` closes the loop by hardening the skills from real usage.
