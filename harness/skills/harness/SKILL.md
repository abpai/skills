...
- Use `onboard.md` to project an audited repo into a machine-readable `autonomous-ready` manifest a factory (GarageBand) can consume, gated on the loop-readiness verdict. Emits the manifest plus an onboarding checklist. Schema in `harness/INTERFACES.md`.
